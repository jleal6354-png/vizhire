  ...
  return (
    <main className="overflow-hidden">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="relative">
            <div className="grid gap-5 md:grid-cols-[0.72fr_1fr]">
              <div className="glass hidden rounded-3xl p-6 md:block">
                <p className="text-sm font-black text-slate-500">Traditional Resume</p>
                <div className="mt-6 h-16 w-16 rounded-full bg-slate-200" />
                <div className="mt-6 space-y-3">
                  {[1, 2, 3, 4, 5, 6].map((line) => (
                    <div key={line} className="h-3 rounded-full bg-slate-200" style={{ width: `${92 - line * 8}%` }} />
                  ))}
                </div>
                <p className="mt-8 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Missing the human signal</p>
              </div>
              <div className="glass rounded-3xl p-4 ring-1 ring-viz-200">
                <div className="mb-3 flex items-center justify-between">
                  <p className="font-black text-viz-700">Real Person. Real Potential.</p>
                  <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">Verified</span>
                </div>
                <VideoFrame image={candidates[0].video} name={candidates[0].name} title={candidates[0].title} match={96} />
                <div className="mt-4 flex flex-wrap gap-2">
                  {["Communication 9.4", "Leadership 9.1", "Problem solving 9.2"].map((pill) => (
                    <span key={pill} className="rounded-full bg-viz-50 px-3 py-1 text-xs font-black text-viz-700">{pill}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      <section className="px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl bg-gradient-to-br from-viz-800 via-viz-700 to-viz-500 px-6 py-12 text-center text-white shadow-glow sm:px-10 sm:py-16 lg:px-16 lg:py-20">
          <div className="mx-auto max-w-4xl">
            <h2 className="text-balance text-3xl font-black leading-tight sm:text-5xl lg:text-6xl">
              Your resume shows what you&apos;ve done.
              <span className="block">VizHire helps employers see how you show up.</span>
            </h2>
            <p className="mx-auto mt-6 max-w-3xl text-base leading-8 text-white/78 sm:text-lg">
              See communication style, confidence, professionalism, and trust earlier, so every interview starts with more clarity.
            </p>
          </div>
        </div>
      </section>
