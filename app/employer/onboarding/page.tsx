  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_50%_0%,rgba(109,59,255,0.16),transparent_34%),linear-gradient(180deg,#ffffff,#fbfaff)] px-4 py-6">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="grid min-h-[calc(100vh-7rem)] items-center gap-10 py-10 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
          ...
          <div className="glass overflow-hidden rounded-[2rem] p-4">
            <div className="relative overflow-hidden rounded-3xl">
              <img src={featured.video} alt={`${featured.name} video introduction`} className="h-[520px] w-full object-cover" />
              ...
              <button className="absolute inset-0 m-auto grid h-16 w-16 place-items-center rounded-full bg-white text-viz-700 shadow-soft" aria-label={`Play ${featured.name} intro`}>
                <Play className="ml-1 h-7 w-7 fill-current" />
              </button>
              <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.22em] text-viz-200">Your first wow moment</p>
                <h2 className="mt-2 text-4xl font-black">{featured.name}</h2>
                <p className="mt-2 font-bold text-white/82">{featured.title} · {featured.location}</p>
                <p className="mt-4 max-w-2xl text-sm leading-6 text-white/76">{featured.summary}</p>
                <Link href="/employer/candidates" className="mt-5 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-5 text-sm font-black text-viz-800">
                  Explore candidate discovery <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
            </div>
          </div>
