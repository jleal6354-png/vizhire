...
function InventoryCandidateCard({ candidate }: { candidate: MarketplaceCandidate }) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-viz-100 bg-white shadow-soft transition hover:-translate-y-1 hover:border-viz-200 hover:shadow-glow">
      <Link href={`/employer/candidates/${candidate.id}`} className="relative block overflow-hidden">
        <img src={candidate.image} alt={`${candidate.name} intro`} className="vh-candidate-thumb h-32 w-full transition duration-500 group-hover:scale-105" />
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <ComingSoonButton className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-viz-50 text-viz-700" ariaLabel={`Save ${candidate.name}`} title="Candidate saved" message="Saved candidate lists are coming soon. Open the candidate profile to review them in detail.">
        ...
        <div className="mt-3 flex items-center justify-between gap-2 border-t border-viz-50 pt-3">
          ...
          <Link href={`/employer/candidates/${candidate.id}`} className="inline-flex items-center gap-1 text-[11px] font-black text-viz-700">
        ...
        <section className="rounded-3xl border border-viz-100 bg-white p-3 shadow-soft">
          <div className="grid gap-3 xl:grid-cols-[1fr_auto] xl:items-center">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 xl:pb-0">
              ...
              <ComingSoonButton className="inline-flex shrink-0 items-center gap-2 rounded-full bg-white px-3 py-1.5 text-[11px] font-black text-viz-700 ring-1 ring-viz-100 transition hover:bg-viz-50" title="Advanced filters" message="Advanced sourcing filters are coming soon. Use the industry tabs to explore live inventory now.">
