  ...
  return (
    <AppShell title="Candidate Review" subtitle="Understand real candidates for this job.">
      ...
      <section className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-4">
          <article className="overflow-hidden rounded-3xl bg-midnight text-white shadow-glow">
            <div className="relative aspect-[16/10] min-h-[300px] max-h-[500px] sm:aspect-video sm:min-h-[260px]">
            <div className="vh-candidate-media absolute inset-0" style={{ "--candidate-image": `url(${featured.candidate.video})` } as React.CSSProperties}>
              <img src={featured.candidate.video} alt={`${featured.candidate.name} video introduction`} />
            </div>
            ...
            <ComingSoonButton
              className="absolute left-1/2 top-1/2 grid h-16 w-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-white text-viz-700 shadow-soft transition hover:scale-105 sm:h-18 sm:w-18"
              ariaLabel={`Play ${featured.candidate.name} intro video`}
              title="Video playback"
              message="Full video playback is coming soon. This preview represents the candidate intro review experience."
            >
          ...
          <div className="hidden rounded-3xl bg-white p-4 shadow-soft sm:block">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[1.25fr_0.9fr_0.95fr_0.65fr]">
              <Link href={`/employer/jobs/${job.id}/review?stage=Interviewing`} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-viz-700 px-5 text-sm font-black text-white shadow-glow"><Video className="h-4 w-4" /> Move to Interview</Link>
              <Link href={`/employer/jobs/${job.id}/review?stage=Shortlisted`} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-viz-50 px-5 text-sm font-black text-viz-700"><ThumbsUp className="h-4 w-4" /> Shortlist</Link>
              <Link href={upNext[0] ? `/employer/candidates/${upNext[0].candidate.id}?job=${job.id}` : `/employer/jobs/${job.id}/review`} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-viz-100 bg-white px-5 text-sm font-black text-slate-700">Next Candidate <ArrowRight className="h-4 w-4" /></Link>
              <Link href={`/employer/jobs/${job.id}/review?stage=Passed`} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-red-100 bg-red-50 px-4 text-sm font-black text-red-600"><X className="h-4 w-4" /> Pass</Link>
            </div>
          </div>
                ...
                return (
                  <Link key={candidate.id} href={`/employer/candidates/${candidate.id}?job=${job.id}`} className="group w-[190px] shrink-0 overflow-hidden rounded-2xl bg-white p-2 shadow-soft transition hover:-translate-y-1 hover:shadow-glow lg:w-[205px]">
                    <div className="relative overflow-hidden rounded-xl">
                      <img src={candidate.video} alt={`${candidate.name} video preview`} className="vh-candidate-thumb h-28 w-full transition group-hover:scale-105" />
        ...
        <aside className="space-y-4 xl:sticky xl:top-28">
          <div className="glass rounded-3xl p-5">
            ...
            <div className="mt-4 grid gap-2.5">
              ...
              <Link href={`/employer/candidates/${featured.candidate.id}?job=${job.id}#resume`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-viz-100 bg-white px-4 text-sm font-black text-slate-600"><Download className="h-4 w-4" /> Resume</Link>
              <Link href={`mailto:?subject=${encodeURIComponent(`VizHire candidate: ${featured.candidate.name}`)}&body=${encodeURIComponent(`Review ${featured.candidate.name}: /employer/candidates/${featured.candidate.id}?job=${job.id}`)}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-viz-100 bg-white px-4 text-sm font-black text-slate-600"><Share2 className="h-4 w-4" /> Share Candidate</Link>
              <ComingSoonButton className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-viz-100 bg-white px-4 text-sm font-black text-slate-600" title="Candidate saved" message="Saved candidate lists are coming soon. This action will keep promising candidates in your recruiter workspace."><Star className="h-4 w-4" /> Save Candidate</ComingSoonButton>
            </div>
          </div>
          ...
          <div className="glass rounded-3xl p-5">
            ...
            <ComingSoonButton className="mt-3 w-full rounded-xl border border-viz-200 bg-white px-4 py-3 text-sm font-black text-viz-700" title="Note saved" message="Persistent hiring notes are coming soon. This will save private feedback for your team.">Save note</ComingSoonButton>
          </div>
        </aside>
      </section>
      <div className="fixed inset-x-0 bottom-[76px] z-30 grid grid-cols-3 gap-2 border-t border-viz-100 bg-white/95 p-3 shadow-glow backdrop-blur sm:hidden">
        <Link href={`/employer/jobs/${job.id}/review?stage=Interviewing`} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-viz-700 px-3 text-xs font-black text-white">Interview</Link>
        <Link href={upNext[0] ? `/employer/candidates/${upNext[0].candidate.id}?job=${job.id}` : `/employer/jobs/${job.id}/review`} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-viz-50 px-3 text-xs font-black text-viz-700">Next</Link>
        <Link href={`/employer/jobs/${job.id}/review?stage=Passed`} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-red-100 bg-red-50 px-3 text-xs font-black text-red-600">Pass</Link>
      </div>
      </>
      )}
    </AppShell>
