        ...
        <section className="glass rounded-3xl p-5 sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-4">
            {visibleCandidates.map(({ candidate, status, jobId }) => (
              <Link key={`${selectedStage}-${candidate.id}-${jobId}`} href={`/employer/candidates/${candidate.id}?job=${jobId}`} className="group overflow-hidden rounded-2xl border border-viz-100 bg-white shadow-soft transition hover:-translate-y-1 hover:border-viz-200 hover:shadow-glow">
                <div className="relative">
                  ...
                  <span className="absolute right-2 top-2 rounded-full bg-white/94 px-2.5 py-1 text-[11px] font-black text-emerald-700">{candidate.match}%</span>
                ...
                <div className="p-2.5">
                  ...
                  <div className="mt-2.5 flex items-center justify-between gap-2 border-t border-viz-50 pt-2.5">
                    <p className="inline-flex items-center gap-1 text-[11px] font-black text-viz-700">
                      <Sparkles className="h-3.5 w-3.5" /> Video
                    </p>
                    <p className="inline-flex items-center gap-1 text-[11px] font-black text-viz-700">
                      <ShieldCheck className="h-3.5 w-3.5" /> {candidate.references} refs
                    </p>
                  </div>
                </div>
              </Link>
